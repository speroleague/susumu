...
fn has_records_other_than(source: &str, allowed_kind: &str) -> bool {
    source
        .split(';')
        .filter_map(|statement| statement.split_whitespace().next())
        .any(|kind| kind != allowed_kind)
}
