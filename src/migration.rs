...
pub fn source_migrations(old: &ProjectAnalysis, new: &ProjectAnalysis) -> Vec<SourceMigration> {
    ...
    for (old_file, new_file, file_confidence) in file_matches {
        migrations.extend(symbol_migrations(old, new, old_file, new_file));
        migrations.extend(workflow_migrations(
            old,
            new,
            old_file,
            new_file,
...
fn symbol_migrations(
        ...
        .filter(|symbol| symbol.file_id == old_file.id)
    {
        ...
        let exact =
            old_symbol.content_hash.is_some() && old_symbol.content_hash == new_symbol.content_hash;
...
fn workflow_migrations(
    old: &ProjectAnalysis,
    new: &ProjectAnalysis,
    old_file: &SourceFile,
    new_file: &SourceFile,
    file_confidence: MigrationConfidence,
) -> Vec<SourceMigration> {
