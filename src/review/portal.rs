...
pub(crate) fn config_style(config: &PortalConfig) -> String {
    ...
    let declarations = config
        ...
        .collect::<Vec<_>>()
        .join(";");
    // `:root:root` outranks the built-in light/dark palette selectors so branded
    // colors win in both themes without needing `!important`.
    format!(":root:root{{{declarations}}}")
}
...
function dirtyBadge(e){return expectationDirtyFindings(e).length?'<span class="tag warning" title="A linked verification or decision was recorded against changed evidence">&#9888; evidence changed</span>':''}
...
function updateThemeToggle(){const btn=$('themeToggle');if(!btn)return;const next=activeTheme()==='light'?'dark':'light';btn.textContent=next==='light'?'☀ Light':'☽ Dark';btn.title='Switch to '+next+' theme';}
