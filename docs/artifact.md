...
Review relationships are checked like other authored links. Susumu emits `SUS050` when a file, symbol, or workflow review thread has no subject, `SUS051` when its target is missing, `SUS052` when a project-wide thread incorrectly carries a subject, `SUS053` when a reply's parent thread is missing, and `SUS054` when parent links form a cycle. Source-revision comparisons emit `SUS056` when an authored record still points at an old file, symbol, or workflow identity. These findings request repair of the discussion or source anchor; they do not change authored status or retarget records.
