...
pub fn write_susu(analysis: &ProjectAnalysis, minified: bool) -> Result<String> {
    ...
    statements.push(format!("susu version={}", analysis.schema_version));
    ...
    for workflow in &analysis.workflows {
        statements.push(format!(
            ...
            workflow.entry_symbol.as_deref().unwrap_or("-"),
            workflow.file_id,
            workflow.confidence,
            workflow.location.start_token(),
            workflow.location.end_token()
        ));
    }
    ...
    for verification in &analysis.verifications {
        statements.push(verification_statement(verification)?);
    }
    ...
    for work in &analysis.works {
        statements.push(work_statement(work)?);
    }
    ...
    let mut output = statements.join(separator);
    output.push(';');
    if !minified {
        output.push('\n');
    }
...
fn quote(value: &str) -> Result<String> {
    serde_json::to_string(value).context("could not encode .susu string")
}
